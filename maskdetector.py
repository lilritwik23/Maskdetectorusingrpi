...
while True:
    ...
    for (box,predicitons) in zip(locations,predicitons):
        (X1,Y1,X2,Y2)=box
        (mask, impropermask,nomask)=predicitons
          
        #labelling
        if mask < impropermask and impropermask>nomask:
            label="Mask on. Thank you"
            color=(0,255,0)
        elif mask>impropermask and mask>nomask:
        ...
        cv2.putText(frame, label, (X1-50, Y1 - 10),cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
